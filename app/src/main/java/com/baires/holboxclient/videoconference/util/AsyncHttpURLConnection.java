...
package com.baires.holboxclient.videoconference.util;
...
public class AsyncHttpURLConnection {
  ...
  public interface AsyncHttpEvents {
    ...
    void onHttpComplete(String response) throws Exception;
  }
  ...
  private void sendHttpMessage() {
    ...
    for(int i=1;(i<=3&&isFull)||i==1;i++) {
      try {
        ...
        Log.d("sendHttpMessage", "SocketTimeoutException: "+e.getStackTrace());
      } catch (IOException e) {
        events.onHttpError("HTTP " + method + " to " + url + " error: " + e.getMessage());
        Log.d("sendHttpMessage", "IOException: "+e.getStackTrace());
      } catch (Exception e) {
        isFull=true;
        this.url=originalUrl+"_retry"+i;
        Log.d("sendHttpMessage", "isFull: "+e.getMessage()+" Retry:"+this.url);
      }
    }
  }
